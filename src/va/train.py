...
from va.loss import accuracy, standard_loss
from va.model import get_model
...
def main_warmup(
    ...
    filename = f"data/log/clean-{ds_name}-{mod_name}-output.txt"
...
def main(ds_name, mod_name, clean_epochs, args) -> None:
    ...
    # Create file to print training progress
    filename = f"data/log/va-epochs-{ds_name}-{mod_name}-output.txt"
    with open(filename, "a") as f:
        # Initialize the model based on the specified architecture
        start_tot = time.time()  # start recording training time
