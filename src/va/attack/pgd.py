...
from va.attack.base import Attack, LabelMixin
from va.util import (
    batch_clamp,
    batch_multiply,
    clamp,
...
def perturb_iterative(
    ...
    Arguments:
        ...
        delta_init (torch.Tensor): (optional) tensor contains the random
        initialization.
    ...
    """
    delta = delta_init if delta_init is not None else torch.zeros_like(xvar)
...
class PGDAttack(Attack, LabelMixin):
    ...
    def perturb(self, x, y=None):
        """
        Given examples (x, y), returns their adversarial counterparts with an
        attack length of eps.
        Arguments:
        ...
        if self.rand_init:
            if self.rand_init_type == "uniform":
                ...
                raise NotImplementedError(
                    "Only rand_init_type=normal and rand_init_type=uniform "
